 ...
 * ```
 */
export function MainNav({ pathname, items, className }: NavigationProps) {
  return (
    <div className={cn("me-4 hidden items-center justify-center md:flex", className)}>
      <nav className="flex items-center gap-4 font-medium text-sm xl:gap-6">
        {items.map((item: NavItem) => (
          <NavItemRenderer key={item.title} item={item} pathname={pathname} level={1} />
        ))}
      </nav>
    </div>
  );
...
function ChildNavItemRenderer({ item, pathname, level }: NavItemRendererProps) {
  const hasChildren = item.children && item.children.length > 0;
  const isChildActive =
    item.href === pathname ||
