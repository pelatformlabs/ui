...
import { type MotionValue, motion, useInView, useSpring, useTransform } from "motion/react";
...
function Digit({
  ...
  return (
    <div style={{ height: digitHeight }} className="relative w-[1ch] overflow-hidden tabular-nums">
      {Array.from({ length: 10 }, (_, i) => (
        <MyNumber key={i} mv={animatedValue} number={i} digitHeight={digitHeight} />
      ))}
    </div>
  );
}
